...
jQuery.validator.addMethod("phone", function(value, element) {     
	var tel = /(^0[1-9]{1}\d{9,10}$)|(^1[3,4,5,8]\d{9}$)/g;     
	return this.optional(element) || (tel.test(value));     
}, "格式为:固话为区号(3-4位)号码(7-9位),手机为:13,14,15,18号段");	
...
jQuery.validator.addMethod("price", function(b, a) {
	return this.optional(a) || isPriceNumber(b)
}, "请输入正确金额,精确到分");
...
//文件扩展名验证
jQuery.validator.addMethod("ext", function(b, a) {
	return this.optional(a) || isExt(b)
}, "请上传pdf、excel、word或rar文件");
function isExt(fileName){
	var ext = fileName.substring(fileName.lastIndexOf(".")+1).toLowerCase();
	if(ext && (ext=="xls"||ext=="doc"||ext=="xlsx"||ext=="docx"||ext=="pdf"||ext=="rar")){
		return true;
	}else
		return false;
}
//文件上传最大限制
jQuery.validator.addMethod("maxUpload", function(b, a) {
		return this.optional(a) || isMax(a)
	}, "上传数据报告不能超过2M!");
function isMax(obj){
