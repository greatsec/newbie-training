...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="sys:area:edit"><li><a href="${ctx}/sys/area/form?parent.id=1">区域添加</a></li></shiro:hasPermission>
	</ul>
	<tags:message content="${message}"/>
	<table id="treeTable" class="table table-striped table-bordered table-condensed">
		<tr><th width="220" style="text-align: left!important;padding-left: 40px;">区域名称</th><th>区域编码</th><th>区域类型</th><th>备注</th><shiro:hasPermission name="sys:area:edit"><th>操作</th></shiro:hasPermission></tr>
		<c:forEach items="${list}" var="area">
			<tr id="${area.id}" pId="${area.parent.id ne requestScope.area.id?area.parent.id:'0'}">
				<td style="text-align: left!important;padding-left: 30px;"><a href="${ctx}/sys/area/form?id=${area.id}">${area.name}</a></td>
				<td>${area.code}</td>
				...
				<shiro:hasPermission name="sys:area:edit"><td>
					...
					<c:if test="${fns:charNumber(area.parentIds,',')<4}">
