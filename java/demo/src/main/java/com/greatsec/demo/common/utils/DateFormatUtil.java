...
public class DateFormatUtil
{
    ...
    throws ParseException
  {
    if ((dateString.trim().indexOf(" ") > 0) && 
      ...
      return new Timestamp(TIMESTAMP_FORMAT.parse(dateString)
        .getTime());
    if (dateString.trim().indexOf(" ") > 0)
    {
      ...
      return new Timestamp(DATETIME_NOSECOND_FORMAT.parse(
        dateString).getTime());
    }
    if (dateString.indexOf(":") > 0)
    {
      if (dateString.trim().indexOf(":") != dateString.trim()
        .lastIndexOf(":")) {
        return new Time(TIME_FORMAT.parse(dateString)
          .getTime());
      }
      return new Time(TIME_NOSECOND_FORMAT.parse(dateString)
        .getTime());
    }
